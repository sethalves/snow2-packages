...
;; the message-digest framework is from the CHICKEN egg.  the
;; md5 implementation is from chibi.


(define-library (seth message-digest md5)
...
;;       ((< i 0) (list->vector ls))))
  ...
  '#(0     0       #xd76a #xa478 #xe8c7 #xb756 #x2420 #x70db #xc1bd #xceee
    ...
    (define digest-length 16)

    (define (make-context)
      (let (;; 3.3 Step 3. Initialize MD Buffer
        ...
        (define (process-block n)
          ;; (write `(process-block ,n))
          ;; (display " i=") (write i)
          ;; (display " pad=") (write pad)
          ;; (newline)
          (cond
           ((< n 64)
            (let ((len (* 8 (+ i n))))
        ...
        (define (final digest)
          ;; (cond ((> buf-i 0) (process-block buf-i)))
          ;; (process-block 0)
          (process-block buf-i)
          (cond ((>= buf-i 56) (process-block 0)))
          (bytevector-u8-set! digest 0 (remainder A0 256))
          (bytevector-u8-set! digest 1 (quotient A0 256))
          (bytevector-u8-set! digest 2 (remainder A1 256))
          (bytevector-u8-set! digest 3 (quotient A1 256))
          (bytevector-u8-set! digest 4 (remainder B0 256))
          (bytevector-u8-set! digest 5 (quotient B0 256))
          (bytevector-u8-set! digest 6 (remainder B1 256))
          (bytevector-u8-set! digest 7 (quotient B1 256))
          (bytevector-u8-set! digest 8 (remainder C0 256))
          (bytevector-u8-set! digest 9 (quotient C0 256))
          (bytevector-u8-set! digest 10 (remainder C1 256))
          (bytevector-u8-set! digest 11 (quotient C1 256))
          (bytevector-u8-set! digest 12 (remainder D0 256))
          (bytevector-u8-set! digest 13 (quotient D0 256))
          (bytevector-u8-set! digest 14 (remainder D1 256))
          (bytevector-u8-set! digest 15 (quotient D1 256))
          digest)

        (vector init update final)))


    (define (init ctx) ((vector-ref ctx 0)))
    (define (update ctx buf n) ((vector-ref ctx 1) buf n))
    (define (final ctx digest) ((vector-ref ctx 2) digest))
          ...
          (unless the-md5-primitive
                  (set! the-md5-primitive
                        (make-message-digest-primitive
                         make-context digest-length init update final)))
          the-md5-primitive)))))
